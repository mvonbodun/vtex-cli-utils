...
pub fn build_product_specs_file(client: &Client, groups_url: String, category_tree_url: String) -> Result<(), Box<dyn Error>> {
    ...
    let groups = get_vtex_field_groups(client, groups_url);
    let group_lookup = parse_spec_groups(groups);
    ...
    let out_path = "transform/data/out/ProductSpecifications.csv";
    ...
    for line in reader.deserialize() {
        let record: ProdDescAttrRecord = line.unwrap();
        ...
        if !unique_spec_cat.contains(&unique_spec_cat_id) {
            ...
            if !record.name.eq("Brand") {
                let spec = Specification {
                    ...
                    position: Some(1),
                    is_filter: Some(false),
                    is_required: Some(false),
                    is_on_product_details: Some(true),
                    is_stock_keeping_unit: Some(false),
